...
2. **`.agents/skills/`** — skill packages (repo-init, machine-management, npu-fleet-monitor, session-management, remote-toolbox, remote-code-parity, modelscope, vllm-ascend-serving, vllm-ascend-benchmark, ascend-memory-profiling, ascend-profiling-collection, ascend-profiling-analysis, curate-workspace-knowledge, vllm-ascend-graph-debug, vllm-ascend-correctness-validation, vllm-ascend-change-validation, vllm-ascend-performance-regression, vllm-ascend-distributed-debug, ascend-operator-debug, ascend-triton-operator-development, ascend-triton-kernel-validation, ascend-triton-kernel-optimization, ascend-triton-workflow, vllm-ascend-pd-serving), each with SKILL.md and any supporting scripts/, references/, or agents/ files.
