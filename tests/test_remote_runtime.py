...
def test_verify_runtime_uses_docker_exec_when_container_ssh_is_closed(
    ...
    assert result.runtime["transport"] == "docker-exec"
    assert result.runtime["container_endpoint"].startswith("docker-exec://")
...
def test_verify_runtime_does_not_claim_ready_when_all_host_transports_fail(
    ...
    assert result.runtime["transport"] == "unknown"
