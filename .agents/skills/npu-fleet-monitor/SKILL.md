...
uv run --no-project python .agents/skills/npu-fleet-monitor/scripts/manage_monitor.py restart
...
Background launch hides the console on Windows. Start, status and stop compare
the saved birth time and command with the live process. Legacy or mismatched
live PID records remain unverified; restart proceeds only after stop succeeds.
