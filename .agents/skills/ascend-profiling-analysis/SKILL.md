...
- **findings 知识引用**：每个 rollup 组用 `finding_type + summary` 查询 Markdown，按相关性取 top-3，写入 `knowledge_refs`：`{ref, title, excerpt, layer, score}`。保留可回读的引用，不要求知识种类或验证标签。无命中保持空数组。
