...
- `evidence/bubble_windows.jsonl` 每行新增 `soft_attribution`：当 rank 在 `source_index.json` 注册了 `trace_view.json` 时，解析 host 侧事件（`cpu_op` / `python_function` / `AscendCL`，流式解析限量保留）给每个 bubble 附 `soft_root_cause_labels` 候选根因（`possible_sync_or_h2d` / `possible_comm_wait` / `possible_untraced_host_blocking` / `possible_host_launch_lag` / `possible_python_serialization_or_lock` / `insufficient_evidence`，rulebook §11）；缺失时为 `null` 并在 `summary_manifest.json:host_trace.limitations` 与 report.md Limitations 注明。标签是候选，不是断言。
