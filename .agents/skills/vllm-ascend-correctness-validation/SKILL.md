...
7. Use `scripts/correctness_run.py compare` to normalize the evidence into a classification and report. It first checks that both results carry an `execution` block and differ only in declared keys, then consumes an observational comparability certificate. An undeclared difference or a `not-comparable` certificate aborts the comparison instead of being reported as a code regression. Two empty answers are not agreement.
8. Route failures:
