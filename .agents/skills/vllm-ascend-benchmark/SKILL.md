...
- **Sum `/metrics` across every engine label.** With `dp > 1` vLLM exposes one label series per engine. Reading a single series, typically `engine="0"`, reports zero whenever the measured requests were balanced onto another engine — a false zero that looks like a feature being inactive rather than a measurement error. This applies to speculative-decoding acceptance counters, prefix-cache hit counters, and request counters alike.
- **Take counter snapshots around a settled window.** Read `/metrics` before and after the measured workload and difference the two, after the counters have stopped moving from earlier traffic. Absolute counters from an unrelated window, and `*_created` timestamp gauges, do not measure the run.
- **Do not use a concurrency-limited runner's per-item latency** as a cache or performance signal. Under `--max-concurrency` that number includes queueing time, so it moves with the concurrency limit rather than with the change under test.
- **A pair of runs is only comparable when one thing differs.** Before attributing a delta to a change, check the pair with `vllm-ascend-experiment-ledger`; a different container, memory-utilization value, or topology explains a delta on its own. `bench_compare.py` holds serve and bench args identical for exactly this reason.
