...
日期：2026-09-10。本文是下一阶段的实施方向与目标行为，**不表示这些能力已经实现**。用户已确定“宽松、先信任、不增加 Agent 负担”的原则，并认可精简现有流程。当前安装版本仍由 `pyproject.toml` 和 `uv.lock` 描述；实现进度见本文末尾的工作批次。

客户端平台范围：**ARM64 macOS（Apple Silicon）与 x86-64 Windows，CPU-only**。Windows 验证使用原生 PowerShell 5.1/7；Intel Mac 不在本阶段范围。
...
本地经验与公共知识一起返回。所有知识都是参考，不是公理；Agent 结合当前代码和运行事实判断。公共审核体现对公开内容负责，不建立跨层信任等级，也不因 candidate / unreviewed 状态过滤或降权。检索以相关性和已知适用条件为主；出处和审核过程按需说明。明确环境不适用仍可据实区分，未知不能自动排除。公共审核通过不表示证明了硬件事实。
...
| 操作 | 必需输入 | 默认结果 |
|---|---|---|
| `knowledge_query` | `text` | 少量相关摘要、内容引用、来源层和已有审核标签；本地与公共一起返回，均为参考；不可用时明确标记 |
...
5. 普通经验可以带范围未知/报告者观察的表述发布；无法消解的冲突或无法成立的强结论留待补充，不阻塞原开发任务。
...
A 与 B 批次在本工作树实现中推进；C–F 仍为 **planned**。可独立的改造无需等整套知识发布链完成，但不要以修改文档代替实现完成。
...
当前 v2 YAML 和严格计划关联仍存在于已安装代码中。迁移采用新的知识包版本，删除被替代的实现和规则；不维护长期 YAML 兼容引擎。旧知识没有强制迁移要求，需要保留的有用条目可一次性转换。发布、依赖更新和客户端切换要同步体现真实进度。
