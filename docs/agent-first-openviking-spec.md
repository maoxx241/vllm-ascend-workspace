...
后续设计以 [九条设计原则](design-principles.md) 为准，完成用户实际目标的总成本优先；组件拆分和现有流程均可调整。执行架构的新方向见 [核心与 coordinator 重设计](vaws-core-redesign.md)；固定执行输入和限定范围的复用已在 coordinator 0.4 实现，其余提案与当前 API 分开说明。本文保留此前实施批次和知识能力的说明，不将其中的流程或组件形态上升为固定约束；替换旧接口时同步修改调用方，不保留兼容层。
