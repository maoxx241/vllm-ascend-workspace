# 个人 Fork 与 Release 更新

Status: current

目标是首次真正使用仓库时即可建立个人开发配置，后续自动准备正式版本。
...
| 原生 CLI 入口 | 本地配置检查，缺身份时一次可见提示 | 后台准备，新建编辑副本可使用准备好的 Release |
...
官方 workspace Release tag 固定整个源码提交，其 pyproject.toml、uv.lock、
vaws-top wheel pin 和 submodule gitlink 一起确定版本，不另复制一套 SHA 清单。
组件单独发布后，维护者更新 workspace 锁定输入并验证，再发布 workspace。
共享知识内容的 Release 继续由 knowledge package 自己同步。

`.github/workflows/release.yml` 在官方仓库收到 `vMAJOR.MINOR.PATCH` tag 时，
...
配置个人身份后，原生入口唤起独立 watcher，默认约每 5 分钟检测官方稳定
Release。没有正式 Release 就报告 no_stable_release，不回退追踪 main。
只有联网且进程运行时有该检测时效；睡眠、离线或进程终止后，由下次原生入口
补启动。它不是秒级推送，也没有声称已经安装开机启动服务。

后台在独立目录准备精确 Release SHA，调用该版本的既有
`vaws_deps.py sync --locked` 复用不可变环境，并缓存/验证 vaws-top wheel。
个人 Fork 默认分支仅 fast-forward 到 Release。普通 `gh repo sync` 跟随
上游分支头，可能超出正式版本，因此不作为这里的实现。

watcher 不改现有工作目录。默认 CLI 新建编辑副本时，若现有来源是干净、
可快进的默认分支，可使用准备好的 Release 作为新副本来源并运行其客户端接线。
已有目录的显式维护可用 apply：要求干净默认分支、没有 merge/rebase，
已初始化子模块无业务改动；只采用 Release gitlink，未初始化子模块保持原样。
...
`.vaws-local/updates/state.json` 保留检测到的 Release、当前步骤、准备结果和
...
Fork 的 Actions 不会自动订阅上游 Release，定时工作也可能延迟或停用。
