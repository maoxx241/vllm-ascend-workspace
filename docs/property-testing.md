...
| toolbox artifacts | local manifests are deterministic and mutation-sensitive; `artifact_pull` never lands disagreeing bytes and refuses manifest relpaths that would escape `local_dir` | remote-dev `tests/test_property_artifacts.py` (the former scaffold duplicate was deleted with the toolbox transport layer) |
...
| Python validator accepted `schema_version: true`/`1.0` and unknown artifact keys | `#94` | `test_known_defect_validator_accepts_schema_version_true_and_float`, `test_known_defect_validator_accepts_artifact_shapes_the_schema_forbids` |
