...
- On a concrete practical failure, query with `knowledge_query` before repeating diagnosis. After a useful conclusion, reuse that summary with `knowledge_capture` (title + content). Do not write a second summary for the knowledge store.
