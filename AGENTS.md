...
- Knowledge is federated across three layers: `shared` (read-only cache of `vllm-ascend-workspace/vaws-knowledge` under `.vaws-local/knowledge/shared/`), `project` (`.agents/knowledge/`, both v1 `<kind>.yaml` and v2 `<kind>.v2.yaml`), and `candidate` (unreviewed local observations). A v2 entry has exactly one body: `rule` or `measurement`; `content_hash` covers `scope` plus that body. `knowledge_query.py` reads all three layers, names the layer and body per match, and reports any layer it could not consult — a degraded answer is never an authoritative "no".
